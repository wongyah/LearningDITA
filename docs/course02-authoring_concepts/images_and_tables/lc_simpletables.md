...
2. 在 `<conbody>` 元素中，将一个 `<simpletable>` 元素插入 `<fig>` 元素的后面：
...
3. 在 `<simpletable>` 元素中，插入一个 `<sthead>` 元素如下：
...
4. 在 `<sthead>` 元素中，插入两个 `<stentry>` 元素，并添加内容如下：
...
5. 在 `<sthead>` 元素的后面，插入一个 `<strow>` 元素如下：
...
6. 在 `<strow>` 元素中，插入两个 `<stentry>` 元素，并添加内容如下：
...
7. 在第一个 `<strow>` 元素的后面，插入两个数据行，并添加内容如下：
