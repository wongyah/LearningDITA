...
6. 在 `<menucascade>` 元素的后面，依次插入一个 `<p>` 元素和一个 `<dl>` 元素如下：
    ...
    <concept id="concept_elements">
     ...
     <conbody>
      ...
      <p>根据体重，家鸭分为不同的种类。有了这些体重等级，你就可以基于鸭子所需的喂食量来选择鸭子品种。</p>
      <dl>
      </dl>
     </conbody>
    </concept>
...
9. 在 `<dlentry>` 元素的后面，再插入三个 `<dlentry>` 元素，并添加内容如下：
    ...
    <concept id="concept_elements">
     ...
     <conbody>
      ...
      <dlentry>
       <dt>轻型鸭</dt>
       <dd>体重第二轻、产蛋最好的鸭种，例如卡基·康贝尔鸭。</dd>
      </dlentry>
      <dlentry>
       <dt>中型鸭</dt>
       <dd>体重中等、性情通常最温顺的鸭种，例如瑞典鸭。</dd>
      </dlentry>
      <dlentry>
       <dt>重型鸭</dt>
       <dd>体重最重、性情通常最友善的鸭种，例如北京鸭。</dd>
      </dlentry>
      ...
     </conbody>
    </concept>
