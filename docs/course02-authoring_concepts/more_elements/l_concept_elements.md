# 参考答案

## 随堂练习的参考答案

本讲中[随堂练习]的参考答案如下：

```xml
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE concept PUBLIC "-//OASIS//DTD DITA Concept//EN" "concept.dtd">
<concept id="concept_elements">
  <title>给小鸭子喂食</title>
  <conbody>
    <p>鸭子出生后的头<u>两个星期</u>，每天<b>24小时</b>随时需要<i>进食</i>和<i>喝水</i>。</p>
    <p>鸭子出生后的头<varname>两个星期</varname>，每天<term>24小时</term>随时需要<cite>进食</cite>和<cite>喝水</cite>。</p>
    <p>由于鸭饲料又干又硬，小鸭子需要充足的水（H<sub>2</sub>O）来保持喙部清洁。</p>
    <p>你可以用回收的塑料容器自制矮矮的、防溢出的喂食器，也可以从 E-Z-Feed<sup>2</sup> 购买现成的产品。</p>
    <p>小鸭子饮食中需要的蛋白质含量比成年鸭要高。<fn>对于刚出生的小鸭子，推荐饮食中的蛋白质含量为18-20%。</fn></p>
    <p>你可以使用电子表格来记录你需要为小鸭子补充鸭粮和水的时间，点击<menucascade>
        <uicontrol>文件</uicontrol>
        <uicontrol>新建</uicontrol>
        <uicontrol>电子表格</uicontrol>
      </menucascade>.</p>
    <p>家鸭可以根据体重分为不同的等级。有了这些体重等级，你就可以基于鸭子的食量来选择鸭子品种。</p>
    <dl>
      <dlentry>
        <dt>矮脚鸭</dt>
        <dd>体重最轻、飞行能力最强的鸭子，比如绿头鸭。</dd>
      </dlentry>
      <dlentry>
        <dt>轻型鸭</dt>
        <dd>体重第二轻、产蛋最好的鸭种，比如卡基·康贝尔鸭。</dd>
      </dlentry>
      <dlentry>
        <dt>中型鸭</dt>
        <dd>体重中等、性情通常最温顺的鸭种，比如瑞典鸭。</dd>
      </dlentry>
      <dlentry>
        <dt>重型鸭</dt>
        <dd>体重最重、性情通常最友善的鸭种，比如北京鸭。</dd>
      </dlentry>
    </dl>
  </conbody>
</concept>
```

## 课后练习的参考答案

本讲中[课后练习]的参考答案如下：

```xml
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE concept PUBLIC "-//OASIS//DTD DITA Concept//EN" "concept.dtd">
<concept id="concept_elements_exercise">
  <title>提升产品的关注度</title>
  <conbody>
    <p>技术内容能够帮助组织提升其产品在市场中的关注度。 理论上说，技术内容的目标受众是**产品的客户**，即那些购买产品后查阅产品文档的人。</p>
    <p>然而一项民意调查显示，大约有三分之一的客户<fn>《客户对产品文档的看法》，由莎伦·伯顿开展的一项在线民意调查。</fn> 会在购买产品前先看文档，而文档的质量会影响他们的购买决策。</p>
    <p>要吸引潜在客户，技术内容必须做到以下三点： 可搜索、易查找、易发现。</p>
    <dl>
      <dlentry>
        <dt>可搜索</dt>
        <dd>客户可以通过网络搜索找到您的内容。</dd>
      </dlentry>
      <dlentry>
        <dt>易查找</dt>
        <dd>客户可以通过相关关键词快速检索到您的内容。</dd>
      </dlentry>
      <dlentry>
        <dt>易发现</dt>
        <dd>您的内容能让人们主动链接或引用。</dd>
      </dlentry>
    </dl>
    <p>想要阅读更多内容，请前往<menucascade>
        <uicontrol>目录</uicontrol>
        <uicontrol>商业目标</uicontrol>
        <uicontrol>营销与产品关注度</uicontrol>
        <uicontrol>提升产品的关注度</uicontrol>
      </menucascade>.</p>
  </conbody>
</concept>
```

[随堂练习]: lc_inline.md#_2
[课后练习]: lc_elements.md#_3