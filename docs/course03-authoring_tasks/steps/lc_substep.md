...
在 DITA 中，操作流程最多只能分为两个层级。第一层级的操作流程使用 `<steps>` 元素；第二层级的操作流程使用 `<substeps>` 元素；不允许插入第三层级的操作流程。当你想插入第三层级的操作流程时，不妨重新考虑一下整个操作流程的层级安排。或许，将这个操作流程拆分成一系列单独的任务型主题会更好。当你在子步骤中进行大量的细节说明时，处理方式同上。
